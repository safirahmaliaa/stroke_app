...
State rendering:
  seg_pending=True  → tampil klasifikasi + loading segmentasi
  seg_pending=False → tampil semua hasil lengkap
"""
...
def _render_clf_section(clf_label, clf_prob):
    """Section 1: Hasil Klasifikasi — selalu ditampilkan."""
...
def _render_input_section(mode, dwi_display, image_array):
    """Section 2: Citra Input — selalu ditampilkan."""
    st.markdown("### 2. Citra Input")

    if mode == "quick" and image_array is not None:
        col_img, _ = st.columns([1, 2])
        with col_img:
            st.image(image_array, caption="Gambar input (Quick Analysis)",
                     use_container_width=True)

    elif mode == "clinical" and dwi_display is not None:
        n_slices = dwi_display.shape[2]
        z_sel    = st.slider(
            "Pilih Slice Axial (DWI)", 0, n_slices - 1,
            find_best_axial_slice(dwi_display, "brightness"),
            key="slice_slider",
        )
        ...
        display_items = [("dwi_display","DWI"), ("adc_display","ADC"), ("flair_display","FLAIR")]
        for i, (key, label) in enumerate(display_items):
            ...
            if vol is not None:
                ...
                cols_view[i].image(PILImage.open(buf), use_container_width=True)
...
def render():
    ...
    with col_back:
        if st.button("← Kembali ke Analisis"):
            set_state("current_page", "analysis"); st.rerun()
    with col_new:
        if st.button("🔄  Analisis Baru", type="primary"):
            clear_all(); set_state("current_page", "home"); st.rerun()
    ...
    # ── Section 1 & 2: selalu tampil ──────────────────────────────────────────
    _render_clf_section(clf_label, clf_prob)
    st.divider()
    _render_input_section(mode, dwi_display, image_array)

    # ── Segmentasi: cek apakah pending atau sudah selesai ─────────────────────
    seg_pending = get_state("seg_pending")
    seg_done    = get_state("seg_done")

    if seg_pending:
        # ── Tampilkan loading block, jalankan segmentasi, lalu rerun ──────────
        st.divider()
        st.markdown("### 3. Segmentasi Lesi — Overlay")

        status_box = st.status(
            "⏳ Menjalankan segmentasi NVAutoNet...", expanded=True, state="running"
        )
        with status_box:
            from pages.analysis import run_segmentation_phase
            run_segmentation_phase()
            status_box.update(label="✅ Segmentasi selesai!", state="complete")

        st.rerun()

    elif seg_done:
        # ── Tampilkan hasil segmentasi lengkap ─────────────────────────────────
        ...
        if vol_ml is not None and sev_label is not None:
            st.divider()
                ...
                <div style="background:#1E1E2E; border-radius:10px; padding:1rem;
                    ...
                    <table style="width:100%; border-collapse:collapse;">
                        <tr>
                            ...
                            <td style="padding:4px 8px; font-weight:700; color:#E0E0E0;">
                                {vol_ml:.2f} mL</td>
                        </tr>
                        <tr>
                            ...
                            <td style="padding:4px 8px; font-weight:700;
                                ...
                                {SEVERITY_LABELS[sev_label]['label'].upper()}</td>
                        </tr>
                        <tr>
                            <td style="padding:4px 8px; color:#9E9E9E;">Threshold</td>
                            <td style="padding:4px 8px; color:#E0E0E0;">
                                Ringan &lt;10mL | Sedang 10–70mL | Berat &gt;70mL</td>
                        </tr>
                        <tr>
                            <td style="padding:4px 8px; color:#9E9E9E;">Segmentasi</td>
                            <td style="padding:4px 8px; color:#E0E0E0;">
                                NVAutoNet (DeepISLES) — Mean DSC 0.8347</td>
                        </tr>
                    </table>
                </div>
                """, unsafe_allow_html=True)

    # ── Section 5: Ringkasan — hanya tampil setelah semua selesai ─────────────
    if not seg_pending:
        st.divider()
        st.markdown("### 5. Ringkasan Hasil")

        lines = [f"**Hasil Klasifikasi:** {clf_label} (probabilitas: {clf_prob*100:.1f}%)"]
        ...
        if seg_done:
            ...
            lines.append("**Segmentasi:** Membutuhkan file ADC dan GPU yang tersedia.")
        elif mode == "quick":
            lines.append("**Catatan:** Analisis volumetrik memerlukan data NIfTI (mode Clinical MRI).")
        ...
        <div style="background:#1A1A2E; border-left:3px solid #FF9800; border-radius:0 8px 8px 0;
            ...
            Hasil ini merupakan output CAD dan bukan keputusan medis final.
            Interpretasi klinis tetap menjadi wewenang dokter yang berkompeten.
        </div>
        """, unsafe_allow_html=True)
